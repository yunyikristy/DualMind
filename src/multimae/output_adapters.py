...
class TokenLearnerAdapter(nn.Module):
    """
    TokenLearner adapter
    ...
    def __init__(self,
                 num_tokens: int,
                 dim_tokens_enc: Optional[int] = None,
                 token_learner_type: str = 'RGB',
                 use_mean_pooling: bool = True,
                 norm_layer: nn.Module = partial(nn.LayerNorm, eps=1e-6),
                 ):
        super().__init__()
        self.num_tokens = num_tokens
        self.token_learner_type = token_learner_type
        self.dim_tokens_enc = dim_tokens_enc
        self.use_mean_pooling = use_mean_pooling
        self.norm_layer = norm_layer
        if self.dim_tokens_enc is not None:
    ...
    def init(self, dim_tokens_enc: int = 768):
        ...
        self.tklr = TokenLearnerModuleV11(in_channels=self.dim_tokens_enc, num_tokens=self.num_tokens, num_groups=4,
                                          dropout_rate=0.)
    ...
    def forward(self,
        ...
        if self.token_learner_type == 'RGB':
            tokens = encoder_tokens[:, :196, :]
        elif self.token_learner_type == "Depth":
            tokens = encoder_tokens[:, 196:392, :]
        tokens = tokens.reshape(-1, 14, 14, 768)
        output_tokens = self.tklr(tokens)
        return output_tokens


class ObservationLinearOutputAdapter(nn.Module):
    """
    Linearxs output adapter.
    :param num_classes: Number of classes
    :param dim_tokens_enc: Dimension of tokens from the encoder
    :param use_mean_pooling: When set to True, uses mean pooling before linear classification head.
        Otherwise, use last token (usually the global token)
